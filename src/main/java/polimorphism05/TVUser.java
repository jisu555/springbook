...
public class TVUser {
	...
	public static void main(String[] args) {
		...
		// 객체 생성을 Spring Framework에서 생성 후 DI 를 통해서 객체를 주입
			...
			// 2. @(어노테이션)을 사용해서 객체를 생성 후 DI (객체를 주입)  <=== Spring Boot 사용하는 방식
				...
				//c. Spring Framework에서 생성된 객체를 의존성 주입(DI)하는 어노테이션 (클래스 변수 위에 할당)
					...
					//@Qualifier : 특정 객체 이름을 사용해서 불러오는것 , @Autowired와 같이 사용함
					
					//@Inject   : 자바에서 제공되는 어노테이션, @Autowired와 동일한 어노테이션 (타입으로 객체를 가지고온다)
					//@Resource : 자바에서 제공되는 어노테이션, <== 이름으로 객체를 식별
							// @Autowired +@Qualifier를 하나로 사용함.
