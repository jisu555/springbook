...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Post 방식으로 변수의 값을 넘길 때 한글 깨어짐 방지 처리
	...
	private void process (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String url = request.getRequestURL().toString();	//URL 정보를 게더링
		...
		if (path.equals("/login.do")) {
			...
			System.out.println("폼에서 넘긴 변수 password값 출력 : " + password);
			...
			if (userD != null) {  //클라이언트에게 전송한 ID와 Pass가 DB의 값과 일치할 때 
			...
			//boardList에는 DB에서 쿼리한 레코드를 담은 DTO 객체가 내장되어있다.
			List<BoardDTO> boardList = dao.getBoardList(dto);
			
			//3. 클라이언트에게 boardList를 전달해야 한다.
			//(세션 객체에 boardList객체를 담아서 전송 시킴)
			// 세션은 서버의 RAM에 저장됨.
			...
			String title = request.getParameter("title");	//inserBoard에서 name값이랑 이름 동일한지 잘 확인
			...
			dao.insertBoard(dto);	//DB에 Insert/Update/Delete가 완료됨
