...
public class BoardDAO {
	...
	//2. SQL 쿼리를 담는 상수에 담아서 처리하는 변수 생성 후 할당 : 상수명 : 전체 대문자로 사용
	...
	//3. 메소드 :
			...
			// getBaord() : BoardDTO에 담아서 전송, 가져온 레코드가 1개
	...
	//3-5. 글 조회 처리 메소드 : getBoardList() : 많은 레코드
		//
	...
	public List<BoardDTO> getBoardList(BoardDTO dto) {
		...
		BoardDTO board ;  //객체를 밖에서 생성하면 모든 값의 하나의 주소만 출력됨 밖에서는 선언만 해야 함.
		...
		try {
			...
			if (rs.next()) {
				do {
					//DTO 객체는 여기서 만들어야함. (별도의 객체에 담기게 됨)
					board = new BoardDTO();
